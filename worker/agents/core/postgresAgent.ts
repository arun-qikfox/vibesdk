...
/**
 * PostgreSQL-based Code Generator Agent
 *
 * This agent uses PostgreSQL for persistence instead of Durable Objects,
 * while maintaining the same interface and behavior as the original agent.
 */
export class PostgresCodeGeneratorAgent extends Agent<Env, CodeGenState> {
    protected projectSetupAssistant: ProjectSetupAssistant | undefined;
    protected sandboxServiceClient: any;
    ...
    private initLogger(agentId: string, sessionId: string, userId: string) {
        this._logger = createObjectLogger(this, 'PostgresCodeGeneratorAgent');
    ...
    constructor(ctx: AgentContext, env: Env) {
        ...
        this.ctx = ctx;
        this.env = env;
        this.agentStateService = new AgentStateService(env);
    ...
    async initialize(
        ...
        try {
            const existingSession = await this.agentStateService.getSession(inferenceContext.agentId);
            if (existingSession && existingSession.state) {
                this.logger().info("Loaded existing agent state from PostgreSQL");
                const codeGenState = existingSession.state as unknown as CodeGenState;
                super.setState(codeGenState);
                return this.state;
            }
        } catch (error) {
            this.logger().warn("Failed to load existing agent state, proceeding with new state", { error });
        }
    ...
    setState(state: CodeGenState): void {
        ...
        try {
            this.agentStateService.saveAgentSession(state.inferenceContext.agentId, state).catch((error) => {
                this.logger().error("Failed to persist agent state to PostgreSQL", { error });
            });
        } catch (error) {
    ...
    getSandboxServiceClient() {
        if (this.sandboxServiceClient === undefined) {
            ...
            const { getSandboxService } = require('../../services/sandbox/factory');
    ...
    async getModelConfigsInfo() {
        ...
        try {
            const modelConfigService = require('../../database/services/ModelConfigService').ModelConfigService;

            // Get all user configs
            const userConfigsRecord = await modelConfigService.getUserModelConfigs(userId);

            // Transform to match frontend interface
            const agents = Object.entries(require('../inferutils/config').AGENT_CONFIG).map(([key, config]) => ({
                key,
                name: config.name,
            ...
            for (const [actionKey, mergedConfig] of Object.entries(userConfigsRecord)) {
                ...
                const defaultConfig = require('../inferutils/config').AGENT_CONFIG[actionKey];
                if (defaultConfig) {
    ...
    /**
     * Get conversation state for user input processing
     */
    ...
    /**
     * Set conversation state after processing user input
     */
        ...
        if (!sandboxInstanceId || redeploy) {
            ...
            if (this.state.commandsHistory && this.state.commandsHistory.length > 0) {
                ...
                if (cmds.length > 10) {
                    ...
                    // because usually LLMs will only generate install commands or rm commands. This is to handle the bug still present in a lot of apps because of an exponential growth of commands
                }
    ...
    async onMessage(connection: Connection, message: string): Promise<void> {
        const { handleWebSocketMessage } = await import('./websocket');
        handleWebSocketMessage(this, connection, message);
    }

    async onClose(connection: Connection): Promise<void> {
        const { handleWebSocketClose } = await import('./websocket');
